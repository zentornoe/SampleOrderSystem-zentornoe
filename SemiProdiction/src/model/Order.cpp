...
void Order::confirm()
{
	if (m_status != OrderStatus::RESERVED)
		throw std::logic_error("confirm은 RESERVED 상태에서만 허용됩니다.");
...
{
	if (m_status != OrderStatus::RESERVED)
		throw std::logic_error("sendToProduction은 RESERVED 상태에서만 허용됩니다.");
...
void Order::completeProduction()
{
	if (m_status != OrderStatus::PRODUCING)
		throw std::logic_error("completeProduction은 PRODUCING 상태에서만 허용됩니다.");
	m_status = OrderStatus::CONFIRMED;
}
...
void Order::release()
{
	if (m_status != OrderStatus::CONFIRMED)
		throw std::logic_error("release는 CONFIRMED 상태에서만 허용됩니다.");
...
{
	if (m_status != OrderStatus::RESERVED)
		throw std::logic_error("reject는 RESERVED 상태에서만 허용됩니다.");
